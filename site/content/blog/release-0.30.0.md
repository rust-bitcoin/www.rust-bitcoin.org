...
First a little excuse for why this is going to be so painful. We have, and do, try to deprecate
things if we make API breaking changes. We do this using
...
2. Replace `Script` with `ScriptBuf` (`s/\([^A-Za-z0-9]\)Script\([^A-Za-z0-9]\)/\1ScriptBuf\2/g` should work in most cases)
...
5. Replace `locktime` with `locktime::absolute`
6. Replace `PackedLockTime` with just `LockTime`
7. Import key types from the `key` submodule rather than `schnorr` or `ecdsa`
8. Replace `SchnorrSighashType` with `TapSighashType`
9. Replace `TapBranchHash` with `TapNodeHash`
10. Fix outstanding compiler errors, if any
11. Optimize the code: replace occurrences of `&ScriptBuf` with `Script`, remove allocations...
12. Remove useless conversions `LockTime` -> `LockTime` (clippy has a lint for it)
...
directory structure to get a feel for things, specifically:
...
- Cryptography related stuff is now in the `crypto` module, all submodules are re-exported so
  you can import them directly (e.g. `use bitcoin::ecdsa`). Modules include: (`ecdsa`, `taproot`,
  `sighash`, `key`)
...
address from a string there were no checks that the address format matched up to the expected
network (e.g. "bc1" prefix for Bitcoin mainnet segwit addresses).
